<template>
  <div>
    ...
    <div class="flex flex-wrap gap-4 mb-4">
      ...
      <!-- เลือกประเทศ -->
      <div>
        <label for="country" class="block mb-2 font-medium">เลือกประเทศ:</label>
        <select
          id="country"
      ...
      <!-- เลือกการเรียงลำดับ -->
      <div>
        <label for="order" class="block mb-2 font-medium">เรียงลำดับ:</label>
        <select
          id="order"
    ...
    <div v-if="stickerData && stickerData.data">
      ...
      <div class="flex justify-between items-center mt-6">
        ...
        <span>หน้าปัจจุบัน: {{ stickerData.current_page }}</span>
        <button
          @click="changePage(stickerData.current_page + 1)"
...
async function fetchStickers(query) {
  try {
    ...
    const url = `https://line2me.in.th/api/sticker-more?${query}`;
